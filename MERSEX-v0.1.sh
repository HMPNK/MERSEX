...
awk '{if($1!="INPUT:" && $1!="OUTPUT:"){print};if($1=="OUTPUT:"){exit;}}' union-op-def.file | awk 'BEGIN{text="start";} {text=text" "$1;} END{print "paste "text; }' | bash | parallel --tmpdir ./ --keep-order -j 16 -l 1000000 --pipe 'mawk -v cmin1=2 -v cmin2=1 -v fcount=$FCOUNT -f count.samples.awk - ' | pigz -c > FINAL-table.tsv.gz
