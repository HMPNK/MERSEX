...
#settings (currently optimized for server with >=256 GB RAM, >=1TB FREE DISK, >=96 CPU THREADS)
KMCTHREADS=20 #kmc kmer counting of fastq files, number of threads
KMCJOBS=4     #kmc kmer counting of fastq files, number of kmc jobs running in parallel
COUNTJOBS=8   #jobs running in parallel, if counting kmer frequencies over all detected kmers in all samples
MERGEJOBS=16  #jobs running in parallel, if merging all kmer frequencies in final table
PVAL=0.01     #P-value cut off, vales equal or smaller than this go to signifcant sex differences table
LOWCOV=0      #Set to 1 for low sequencing coverage data (1-6x) per individual (genome skimming approaches), may increase runtime and disk usage!
...
ls *.list | awk -v cpu=$KMCTHREADS '{print "kmc -t"cpu" -k27 -r @"$1" "$1".res ./kmc_tmp_dir/ > "$1".res.log 2>&1"}' > KMC_BATCH.sh
...
ls *list | awk '{n=$1;gsub(".list","",n);print "kmc_tools simple allunion-nocounts -ci0 "$1".res -ci0 union all_"n" -ocright; kmc_dump all_"n" /dev/stdout | cut -f 2 | pigz -c > all_"n".counts.gz; rm all_"n"*kmc_*"}' | nohup parallel -j $COUNTJOBS
...
awk '{if($1!="INPUT:" && $1!="OUTPUT:"){print};if($1=="OUTPUT:"){exit;}}' union-op-def.file | awk 'BEGIN{text="start";} {text=text" "$1;} END{print "paste "text; }' | bash | parallel --tmpdir ./ --keep-order -j $MERGEJOBS -l 1000000 --pipe 'mawk -v cmin1=2 -v cmin2=1 -v fcount=$FCOUNT -f count.samples.awk - ' | mawk -v pval=$PVAL '{if(pval!="" && $1<=pval) {print | "pigz -c > Pval"pval".tsv.gz"}; print | "pigz -c > FINAL-table.tsv.gz"}'
